...
const defaultEvent = () => {
	...
	newEvent.agenda = [{ description: "", start: newEvent.startDateTime, end: addMinutes(newEvent.startDateTime, newEvent.duration), id: shortid.generate() }] as AgendaItem[];
	newEvent.invites = []
	return newEvent
}
...
export default function EventForm(props: Props) {
	...
	const addAgendaItem = () => {
		...
		newEvent.agenda.push({ description: "", start: event.startDateTime, end: addMinutes(event.startDateTime, event.duration), id: shortid.generate() })
		setEvent(newEvent);
	}
	...
	const saveEvent = () => {
		const id = props.eventId !== undefined ? props.eventId : shortid.generate();
		try {
			...
			<div ref={refs.setReference} className={styles.wrapper}>
				<div className={styles.formContainer}>
					<div className={styles.info}>
						<input type="text" placeholder="Title..." className={styles.title} onChange={
							(changeEvent) => {
								const updatedEvent = cloneDeep(event)
						...
						<textarea placeholder="Description..." className={styles.description} onChange={
							(changeEvent) => {
								const updatedEvent = cloneDeep(event)
						...
						<div className={styles.userInvites}>
							<div className={styles.userContainer}>
								{props.users.filter(u => !event.invites.find(i => i === u)).sort((a, b) => a.name.localeCompare(b.name)).map((user, index) => {
									return <div key={index} className={styles.userInviteButton}>{user.name} <div className={styles.addButton} onClick={() => {
										addUserInvites(user)
									}}>+</div>
									</div>
								})}
							...
							<div className={styles.userContainer}>
								{event.invites.sort((a, b) => a.name.localeCompare(b.name)).map((user, index) => {
									return <div key={index} className={styles.userInviteButton}>{user.name} <div className={styles.removeButton} onClick={() => {
					...
					<div className={styles.agenda}>
						...
						<div className={styles.agendaList}>
							{event.agenda.map((item, index) => {
								return <div key={item.id} className={styles.agendaItemContainer}>
				...
				<div className={styles.buttons}>
					<button className={styles.button} type="button" onClick={() => { props.onClose(); resetEvent(); }}>Close</button>
					<button className={styles.button} type="button" onClick={() => { saveEvent(); }}>Save</button>
				</div>
			</div>
		);
	}
