...
export default function TimeSelector(props: Props) {
	...
	for (let i = 0; i < 24; i++) {
		hourElements.push(<Button variant={hour === i ? "default" : "ghost"} key={i} className={`w-4`} onClick={() => {
	...
	for (let i = 0; i < 12; i++) {
		minuteElements.push(<Button variant={minute === (i * 5) ? "default" : "ghost"} key={i} className="w-4" onClick={() => {
	...
	return <div className="m-2">
		...
		<Separator />
		<div className={`${styles.customScroll} flex justify-evenly h-32 gap-2`}>
