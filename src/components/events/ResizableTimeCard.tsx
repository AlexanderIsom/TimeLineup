...
export default function ResizableTimeCard({ schedule, updateHandler, onContext, bounds }: Props) {
        ...
        <Resizable className={styles.container} width={Timeline.minutesToXPosition(state.duration)} height={0} resizeHandles={["e", "w"]} onResize={onResize} onResizeStop={onResizeStop}>
          <div style={{ width: `${Timeline.minutesToXPosition(state.duration)}px` }}>
            <div className={`dragHandle ${styles.timeContainer} ${styles.grabbable}`}>
              <span className={styles.timeCue}>{format(roundToNearestMinutes(addMinutes(bounds.start, state.offsetFromStart), { nearestTo: Timeline.getSnapToNearestMinutes() }), "HH:mm")}</span>
              <span className={styles.timeCue}>
                {format(roundToNearestMinutes(addMinutes(bounds.start, state.offsetFromStart + state.duration), { nearestTo: Timeline.getSnapToNearestMinutes() }), "HH:mm")}
              </span>
            </div>
          </div>
        </Resizable>
