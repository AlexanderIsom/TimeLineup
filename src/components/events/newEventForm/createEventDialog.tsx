...
export default function CreateEventDialog({ event, isEditing = false }: Props) {
  ...
  return (
    <Drawer open={open} onOpenChange={(value: boolean) => {
      setOpen(value);
      if (value) {
        resetForm();
      }
      ...
      <DrawerContent className="overflow-clip max-h-[96%]">
        ...
        <div className="px-4 overflow-y-auto overscroll-none">
          {formContent}
        </div>
        <DrawerFooter>
          <div className="w-full flex">
            <Button type="button" variant={"ghost"} className={`${currentStep === 0 && "hidden"} w-full`} onClick={() => {
              prev()
            }}>Back</Button>
