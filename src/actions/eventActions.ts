...
export async function UpdateEvent(eventData: NotUndefined<EventDataQuery>, invitedUsers: Array<Profile>) {
	...
	segments.forEach((segment) => {
		if (
			!isWithinInterval(segment.start, {
...
export async function GetLocalUserEvents() {
	...
	const attendingEvents = await db.query.rsvps
		.findMany({
		...
		.then((values) => {
			return values.map((v) => {
				return { ...v.event, status: v.status };
			});
		});

	const hostedEvents = await db.query.events
		.findMany({
...
export async function GetEventData(eventId: string) {
	...
	const attendingEventData = await db.query.rsvps
		.findFirst({
