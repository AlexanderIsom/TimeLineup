...
import { pgTable, timestamp, text, json, varchar, boolean, uuid, pgEnum } from "drizzle-orm/pg-core";
...
export const friendships = pgTable("friendship", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	sending_user: uuid("sending_user")
		.notNull()
		.references(() => profiles.id, { onDelete: "cascade" }),
	receiving_user: uuid("receiving_user")
		.notNull()
		.references(() => profiles.id, { onDelete: "cascade" }),
